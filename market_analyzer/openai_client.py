...
class NewsletterAI:
    ...
    def generate_newsletter(self, newsletter_data: Dict) -> str:
        """Generate a comprehensive newsletter from market data"""
        
        # Create the main prompt
        prompt = self._build_newsletter_prompt(newsletter_data)
        
        try:
            system_message = """You are a financial market analyst who specializes in prediction markets. You write engaging, informative newsletters about market movements that are accessible to both novice and experienced traders. 
...
When searching for context, look for recent news about Federal Reserve policy, geopolitical events, cryptocurrency developments, or other relevant topics that could explain the price movements you're analyzing."""
    ...
    def _build_newsletter_prompt(self, data: Dict) -> str:
...
"""
        ...
        if gainers:
            prompt += "\nBIGGEST GAINERS:\n"
            for market in gainers[:3]:
                prompt += f"- {market['question']}: +{market['change_pct']} ({market['volume']} volume)\n"
        
        if losers:
            prompt += "\nBIGGEST DECLINES:\n"
            for market in losers[:3]:
                prompt += f"- {market['question']}: -{market['change_pct']} ({market['volume']} volume)\n"
...
## Executive Summary
- Use web search to research current events that explain market movements
- Identify 3-6 core themes connecting multiple markets with numbered citations [1], [2], etc.
- Focus on WHY movements are happening, not just what moved
...
**Why it matters for markets:** [Specific market implications and cross-asset effects]
...
- Each theme MUST include the three bolded subsections: "Why it matters for markets", "What's likely driving this", "What to expect next"
- END the newsletter with this exact footer format: "Generated: {data.get('timestamp', 'Unknown')} | {summary_stats.get('total_markets_analyzed', 0)} markets analyzed"
"""
...
"""
        ...
        try:
            system_message = "You are a prediction market analyst who explains market movements with context and insight."
            full_prompt = f"{system_message}\n\n{prompt}"
